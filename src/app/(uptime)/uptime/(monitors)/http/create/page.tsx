  ...
  return (
    ...
    {loading ? (
      ...
      <div className="p-6 m-auto mt-4 border w-[80%] bg-lightGray">
        ...
        <div className="mt-5">
          ...
          <label htmlFor="active" className="block my-1 text-xs text-gray-500">
            This defines whether the test runs right away, or paused currently
        ...
        labelEnd="Headers to be attached to your request to the endpoint"
      ...
      />
        ...
        labelEnd="Body for your request to the endpoint"
    ...
    <select id="auth" name="auth"
        className={clsx( 'bg-white border border-black text-gray-900 text-sm rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'
        , { 'border border-red-400' : validationErrors!.basicAuthUser || validationErrors!.basicAuthPass } )}
        value={monitorInfo.httpAuthMethod} onChange={(event: ChangeEvent)=> {
        const value: string = (event.target as HTMLInputElement).value;
    ...
    <div className="mt-4 border p-4">
        ...
        <>
            <MonitorItem id="text" type="username" topClass="mb-2" labelStart="Username"
                className={clsx( 'bg-white border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'
                , { 'border border-red-400' : validationErrors!.basicAuthUser } )}
                inputValue={monitorInfo.basicAuthUser} placeholder="Username" onChange={(event: ChangeEvent)=> {
                const value: string = (event.target as HTMLInputElement).value;
                setMonitorInfo({ ...monitorInfo, basicAuthUser: value });
                }}
                />
                <MonitorItem id="password" type="password" topClass="mb-2" labelStart="Password"
                    className={clsx( 'bg-white border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'
                    , { 'border border-red-400' : validationErrors!.basicAuthPass } )}
                    inputValue={monitorInfo.basicAuthPass} placeholder="Password" onChange={(event: ChangeEvent)=> {
                    const value: string = (event.target as HTMLInputElement).value;
        ...
        <>
            <MonitorItem id="token" type="text" topClass="mb-2" labelStart="Token"
                className={clsx( 'bg-white border border-black text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5'
                , { 'border border-red-400' : validationErrors!.bearerToken } )} inputValue={monitorInfo.bearerToken}
                placeholder="Bearer token" onChange={(event: ChangeEvent)=> {
                const value: string = (event.target as HTMLInputElement).value;
