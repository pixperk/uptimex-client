...
import { httpSchemaValidation } from '../validations/http';
import { FetchResult, useMutation, useQuery } from '@apollo/client';
import { CREATE_MONITOR, GET_SINGLE_MONITOR, GET_USER_MONITORS, UPDATE_MONITOR } from '@/queries/status';
    ...
    startTransition(async () => {
      try {
        ...
        if (!Object.values(resultSchema).length) {
          ...
          showSuccessToast(
            `Created Monitor : ${updatedMonitor.name} successfully`
          );
        }
      } catch (error) {
        showErrorToast(
          (error as GraphQLError).message || 'Error creating HTTP Monitor'
        );
      }
    });
  };
  ...
  const onHandleSubmit = (): void => {
    startTransition(async () => {
      try {
        ...
        showErrorToast((error as Error).message||'Error updating HTTP Monitor.');
      }
    });
  };
