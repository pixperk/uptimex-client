...
import { postgresSchemaValidation } from '../validations/postgres';


export const usePostgresCreate = (): IUseUptime => {
  const {
  ...
  const updateMonitorValues = (): IMonitorDocument => {
    return {
      ...monitorInfo,
      userId: user?.id!,
      connection: monitorInfo.connection === 'none' || monitorInfo.connection === '' ? 'established' : monitorInfo.connection
    };
  };
  ...
  const onHandleSubmit = (): void => {
    startTransition(async () => {
      try {
        const resultSchema = postgresSchemaValidation(monitorInfo);
        ...
        if (!Object.values(resultSchema).length) {
          ...
          if (result) {
            showSuccessToast(`Created PostgreSQL monitor : ${monitorInfo.name} successfully.`);
            router.push('/status');
          }
        }
      } catch (error) {
        showErrorToast('Error creating PostgreSQL Monitor.');
      }
    });
  };
...
export const usePostgresEdit = (monitorId: string): IUseUptime => {
  const {
    state: { user, notifications },
  } = useContext(MonitorContext);
  const [validationErrors, setValidationErrors] =
  ...
  const onHandleSubmit = (): void => {
    startTransition(async () => {
      try {
        const resultSchema: IMonitorErrorMessage = postgresSchemaValidation(monitorInfo);
        setValidationErrors(resultSchema);
        if (!Object.values(resultSchema).length) {
          const updatedMonitor: IMonitorDocument = updateMonitorValues();
          ...
          if (result) {
            showSuccessToast('Updated PostgreSQL monitor successfully.');
            router.push('/status');
          }
        }
      } catch (error) {
        showErrorToast('Error updating PostgreSQL Monitor.');
      }
    });
  };
