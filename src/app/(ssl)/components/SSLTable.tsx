...
import type { IPagination } from "@/interfaces/monitor.interface"
import { type ISSLInfo, type ISSLMonitorDocument, sslDefaultInfoData } from "@/interfaces/ssl.interface"
...
const SSLTable: FC<SSLTableProps> = ({ monitors, limit }): ReactElement => {
  ...
  return (
    <>
      {showModal && selectMonitor && <SSLInfoModal monitor={selectMonitor} onClose={() => setShowModal(false)} />}
              ...
              return (
                <tr key={monitor.id} className={`${index % 2 !== 0 ? "bg-white" : "bg-[#f8f8fa]"}`}>
                  <th scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                    <button
                      ...
                      className={clsx("mr-1 inline-flex items-center px-4 py-2 text-sm font-bold text-white rounded", {
                        ...
                        "bg-red-400":
                          monitorInfo.type === "danger" || monitorInfo.type === "--" || monitorInfo.type === "expired",
                  ...
                  <td className="px-6 py-5 max-w-[250px] whitespace-nowrap text-ellipsis truncate">
                    {monitorInfo.issuer?.common_name || "N/A"}
                  </td>
                  <td className="px-6 py-5">
                    {monitorInfo.info?.validFrom ? formatSSLDate(monitorInfo.info.validFrom) : "N/A"}
                  </td>
                  <td className="px-6 py-4">
                    {monitorInfo.info?.validTo ? formatSSLDate(monitorInfo.info.validTo) : "N/A"}
                  </td>
                  <td className="px-6 py-4">
                    {monitorInfo.info?.daysLeft !== undefined
