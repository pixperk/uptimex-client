...
export const useHome = (): IUseHome => {
      ...
      if (userId === user?.id) {
        ...
        client.cache.updateQuery(
          ...
          () => {
            return{
              getUserMonitors: {
                __typename: 'MonitorResponse', monitors
              }
            }
          }
        );
  ...
  const refreshMonitors = async (): Promise<void> => {
    try {
      if (hasActiveMonitors) {
        ...
        showErrorToast('There are no active monitors.');
      }
    } catch (error) {
      showErrorToast((error as GraphQLError).message || 'Error in refreshing');
    }
  };
  ...
  const enableAutoRefresh = async (): Promise<void> => {
    try {
      ...
      showErrorToast(
        (error as GraphQLError).message || 'Error enabling auto refresh'
      );
    }
  };
